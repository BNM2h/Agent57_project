...
class Qnet(nn.Module):
    def __init__(self):
        ...
        self.hx=Variable(torch.zeros(1,1,128)).type(FLOAT).to('cuda')
        self.cx=Variable(torch.zeros(1,1,128)).type(FLOAT).to('cuda')
    ...
    def forward(self, x,train=False):
        ...
        if not train:
            ...
            x=self.maxpooling(x) #32*39*51
            x=F.relu(self.bn2(self.conv2(x)))
            x=self.maxpooling(x) #64,9,12
            ...
            hx=Variable(torch.zeros(1,1,128)).type(FLOAT).to('cuda')
            cx=Variable(torch.zeros(1,1,128)).type(FLOAT).to('cuda')
            sequence_length=x.size(1)
